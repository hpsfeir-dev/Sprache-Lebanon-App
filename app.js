...
function speak(text,rate=.78){if(!('speechSynthesis'in window)){toast('Sprachausgabe wird hier nicht unterstützt.');return}speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text);utterance.lang='ar-LB';utterance.rate=rate;speechSynthesis.speak(utterance)}
...
function playDialogue(){const scenario=scenarios.find(item=>item.id===state.scenario);const texts=[scenario.opening.ar,...scenario.responses.map(item=>item.ar)];let i=0;const next=()=>{if(i>=texts.length)return;const u=new SpeechSynthesisUtterance(texts[i++]);u.lang='ar-LB';u.rate=.75;u.onend=next;speechSynthesis.speak(u)};if('speechSynthesis'in window){speechSynthesis.cancel();next()}else toast('Sprachausgabe wird hier nicht unterstützt.')}
